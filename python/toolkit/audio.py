...
def create_output_directory(directory: Path, suffix: str) -> Path:
    ...
    for item in directory.rglob("*"):
        if item.is_file() and item.suffix.lower() not in audio_extensions and item.suffix.lower() not in skip_extensions:
...
def calculate_image_size(path: Path) -> None:
    ...
    for flac_file in path.glob("*.flac"):
        try:
            ...
            raise FileOperationError(f"Failed to check artwork size for {flac_file.name}", flac_file) from e
        except OSError as e:
...
def process_sacd_directory(directory: Path, fmt: AudioFormat = "all") -> None:
    ...
    for folder in tqdm(output_dirs, desc="Converting DFF", unit="disc", position=0, leave=True):
        try:
            ...
            if e.stderr:
                logger.error(f"stderr: {e.stderr[:500]}")
            raise ConversionError(f"DFF conversion failed in {folder}", folder, e.cmd) from e
    ...
    for pattern, suffix, cmd in channel_configs:
        if re.search(pattern, probe_result, flags=re.IGNORECASE):
            ...
            try:
                ...
                logger.error(f"sacd_extract {suffix} failed for {iso_path.name} (exit {e.returncode})")
                if e.stderr:
                    logger.error(f"stderr: {e.stderr[:500]}")
                raise
...
def convert_dff_to_flac(dff_dir: Path, sample_fmt: str = "s32", sample_rate: int = 88200, position: int = 0) -> None:
...
def calculate_gain(dff_file: Path, target_headroom_db: float = -0.5) -> float:
    ...
    try:
        ...
        raise ConversionError(f"ffmpeg volumedetect failed on {dff_file.name}", dff_file) from e
...
def flac_directory_conversion(directory: Path, tier: AudioTier, position: int = 0) -> None:
    ...
    for f in tqdm(flac_files, desc=f"{bit_depth}-bit/{sample_rate // 1000}kHz", position=position, leave=False):
        ...
        try:
            ...
            raise ConversionError(f"SoX conversion failed for {f.name}", f, e.cmd) from e
...
def downsample_flac(source: Path, dest: Path, tier: AudioTier) -> None:
    ...
    try:
        ...
        if dest.exists():
            dest.unlink()
        raise ConversionError(f"SoX conversion failed for {source.name}", source, cmd) from e
...
def convert_to_mp3(directory: Path) -> None:
    ...
    for f in tqdm(flac_files, desc="Converting to MP3"):
        ...
        try:
            ...
            raise ConversionError(f"ffmpeg MP3 conversion failed for {f.name}", f) from e
