...
## Module Organization
The toolkit is organized as a flat set of modules under the `toolkit/` package.
- `cli.py`: The command-line interface entry point.
- `audio.py`, `video.py`, `filesystem.py`, `pristine.py`, `lastfm.py`: Domain-specific logic.
- `cuesheet.py`: Specialized audio parsing logic.
- `utils.py`, `types.py`, `logging_config.py`, `exceptions.py`: Shared core utilities.
