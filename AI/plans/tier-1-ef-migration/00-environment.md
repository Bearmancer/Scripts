...
- Docker Desktop must be running before any EF Core or Testcontainers tests execute
- `$env:PGCONNSTR` must be loaded from `.env` and contain Host=, Database=, Username= parameters
- PostgreSQL 18 is running locally via Docker Compose with connection pooling enabled (default Npgsql pool: 100 max connections)
- `ScriptsDbContext` uses NoTracking as default query behavior (set in constructor via `ChangeTracker.QueryTrackingBehavior = QueryTrackingBehavior.NoTracking`)
- Entity configurations are auto-discovered via `ApplyConfigurationsFromAssembly()` — no manual registration needed
- Connection string must NEVER be logged, echoed, or hardcoded — always use environment variables
