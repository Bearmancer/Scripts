...
## Task 0 — Create Scripts.slnx if absent

**Current State:** `Scripts.slnx` is required by all downstream tasks but may not exist (T1 may not have created it).

**Reason:** The solution file is the container for all 8 modular projects. Without it, `dotnet sln add` fails in every subsequent task.

**What:** Check for existing `Scripts.slnx`. If absent, create via `dotnet new slnx`. If present, back up and verify it's functional.
...
if (Test-Path $slnx) {
    Write-Host "OUTCOME: Scripts.slnx exists — backup and verify"
    $bak = "$slnx.bak.$ts"
    ...
    # Verify
    if (-not (Test-Path $slnx)) { throw "Scripts.slnx was not created" }
    Write-Host "OUTCOME: Scripts.slnx created"
}
```
