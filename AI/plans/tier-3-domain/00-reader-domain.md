...
The Reader domain must follow the inward-only dependency flow: **Reader → Core only**. No direct references to Data, Orchestrators, or any Services.

### Reader Persistence via Interfaces

Reader does not directly instantiate `ScriptsDbContext` or call repository methods. Instead, Reader receives persistence abstractions via DI:

- **IDocumentStore**: Abstraction for persisting parsed document results (e.g., article content, PDF text)
- **IVideoRepository**: Abstraction for storing video metadata extracted from web pages

These interfaces are defined in `Scripts.Core.Abstractions` and implemented in `Scripts.Data.Repositories`. Reader receives them via constructor injection and calls methods like `IDocumentStore.SaveAsync(key, content)` without knowing about the underlying database.

### Reader's Role in the Architecture

Reader is a **pure extraction layer**:
1. Receives URLs/content from Orchestrators
2. Parses/extracts using Playwright, AngleSharp, PdfPig, OCR
3. Returns structured data (ArticleContent, VideoMetadata) to Orchestrators
4. Orchestrators handle persistence via repositories

Reader never calls `DbContext` directly. All persistence is delegated to Orchestrators.
