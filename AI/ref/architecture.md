...
semantic-dependent) with a hybrid **Local PostgreSQL 18 + OCI Object Storage** backup architecture.
...
- **OCI Object Storage** — secure, Always Free cloud backup storage for database snapshots (`pg_dump`) and write-ahead logs (WAL) to support Point-In-Time Recovery (PITR).
- **Cost Protection** — managed via custom OCI quota policy (`always-free-quota-policy`) and block volume backup policy (`rolling-5-policy`), ensuring permanently $0.00 hosting and storage cost.
