...
def list_directories(path: Path, sort_order: str = "0", indent: int = 0):
    ...
    entries = [(entry, get_folder_size(entry))
...
def make_torrents(folder: Path):
    ...
    dropbox = Path(json.load(open(Path.home() / 'AppData' / 'Local' /
                                  'Dropbox' / 'info.json')).get('personal', {}).get('path'))
...
def main():
    parser = argparse.ArgumentParser(
        description="File and Torrent Management Tool")
    parser.add_argument('command', choices=[
        'list_dir', 'list_files_and_dirs', 'make_torrents'], help='Command to execute')
