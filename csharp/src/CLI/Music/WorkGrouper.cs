		...
		void FlushGroup()
		{
			...
			foreach (TrackInfo t in currentGroup)
			{
				if (t.RecordingYear.HasValue)
					years.Add(t.RecordingYear.Value);
				...
				if (!IsNullOrWhiteSpace(t.RecordingVenue))
					venuesSet.Add(t.RecordingVenue);
			}
	...
	private static void DetectMissingWorkHierarchy(List<WorkSummary> works)
	{
		...
		for (var i = 0; i < workCount - 1; i++)
		{
			...
			if (currentColon > 5 && nextColon > 5)
			{
				...
				if (currentPrefix == nextPrefix && suspectedMissing.Add(currentPrefix))
					suspectedMissingInOrder.Add(currentPrefix);
			}
		}
