...
public sealed class MusicFillCommand : AsyncCommand<MusicFillCommand.Settings>
{
	...
	)
	{
		...
		List<RecordingInput> records = ReadRecordings(inputFile);
		...
		var delimiter = output.EndsWith(".tsv") ? '\t' : ',';
		...
		await Console
			...
			.StartAsync(async ctx =>
			{
				...
				foreach (RecordingInput record in records)
				{
					...
					var workName = !IsNullOrEmpty(record.Work) ? record.Work : "(Unknown Work)";
					...
					SuggestionSet suggestions = await SearchForSuggestionsAsync(
						record,
						mbService,
						discogsService,
						cancellationToken
					...
					if (suggestions.HasAny())
					{
						...
						TimeSpan elapsed = fillTimer.Elapsed;
	...
	private static List<RecordingInput> ReadRecordings(string filePath)
	{
		...
		for (var i = 1; i < lines.Length; i++)
		{
			...
			var fields = line.Split(['\t'], headerIndex.Count, StringSplitOptions.None);
	...
	{
		var normalized = Regex.Replace(line.TrimEnd(), "\\s{2,}", "\t");
		var parts = normalized.Split(['\t'], expectedFieldCount, StringSplitOptions.None);

		if (parts.Length >= expectedFieldCount)
			return parts;

		return [.. parts.Concat(Enumerable.Repeat(Empty, expectedFieldCount - parts.Length))];
	}
	...
	private static void DisplayResults(List<RecordingWithSuggestions> results)
	{
		...
		foreach (RecordingWithSuggestions item in results)
		{
			...
			if (item.Suggestions.Items.Count > 0)
			{
				...
				foreach (SuggestionBundle bundle in item.Suggestions.Items)
				{
					var conf =
					...
					var label = IsNullOrEmpty(bundle.Label)
						? "[dim]-[/]"
						: $"[cyan]{Console.Escape(bundle.Label)}[/]";
					var cat = IsNullOrEmpty(bundle.CatalogNumber)
						? "[dim]-[/]"
						: $"[cyan]{Console.Escape(bundle.CatalogNumber)}[/]";
					var year = IsNullOrEmpty(bundle.Year) ? "[dim]-[/]" : $"[cyan]{bundle.Year}[/]";
