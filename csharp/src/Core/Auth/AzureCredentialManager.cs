...
internal static class AzureCredentialManager
{
	...
	public static void EnsureCredentials()
	{
		...
		catch (Exception ex) when (ex is CredentialUnavailableException or AuthenticationFailedException)
		{
			...
			SetEnvironmentVariable("AZURE_CLIENT_SECRET", clientSecret, EnvironmentVariableTarget.Process);
			SetEnvironmentVariable("AZURE_TENANT_ID", tenantId, EnvironmentVariableTarget.Process);

			AnsiConsole.MarkupLine("[green]Process environment variables populated successfully.[/]");
		}
	}
}
