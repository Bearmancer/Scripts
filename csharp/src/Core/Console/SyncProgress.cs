...
internal sealed class SyncProgressRenderer(SyncProgressTracker tracker)
{
	...
	private static string BuildProgressBar(double percent) => UI.WideProgressBar(percent);

	private static string GetBarColor(double percent) => UI.ProgressColor(percent);

	private static string FormatTimeText(SyncProgressSnapshot snapshot) =>
		snapshot.EstimatedTimeRemaining is { } eta
			? $"ETA: {FormatDuration(eta)}"
			: $"Elapsed: {FormatDuration(snapshot.ElapsedTime)}";
