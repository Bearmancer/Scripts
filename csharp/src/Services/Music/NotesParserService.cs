...
internal static partial class NotesParserService
{
	internal static ParsedNotes Parse(string notes)
	{
		...
		foreach (var rawLine in lines)
		{
			...
			if (RecordingContextPattern().IsMatch(line))
			{
				foreach (Match dm in DatePattern().Matches(line))
				{
					...
					recordingDates.Add(
						new RecordingDate(Description: line, Date: TryParseDate(dateText))
					);
				}
			}
		...
		return new ParsedNotes(
			Composers: [.. composers.Distinct(StringComparer.OrdinalIgnoreCase)],
			Conductors: [.. conductors.Distinct(StringComparer.OrdinalIgnoreCase)],
			Orchestras: [.. orchestras.Distinct(StringComparer.OrdinalIgnoreCase)],
			Venues: [.. venues.Distinct(StringComparer.OrdinalIgnoreCase)],
