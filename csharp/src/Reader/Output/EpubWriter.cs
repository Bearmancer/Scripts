...
internal static partial class EpubWriter
{
	...
	public static void Write(ArticleContent content, string outputPath)
	{
		...
		var escapedTitle = WebUtility.HtmlEncode(value: content.Title);
		var authorString =
			content.Authors.Count > 0
				? Join(separator: ", ", content.Authors.Select(selector: WebUtility.HtmlEncode))
				: "Unknown Author";
		var isJstor = content.OriginalPdf is { };
		...
		WriteAssets(zip: zip, content: content);
		WriteText(
			zip: zip,
	...
	private static void WriteAssets(ZipArchive zip, ArticleContent content)
	{
		foreach (KeyValuePair<string, byte[]> kvp in content.Images)
			WriteBinary(zip: zip, $"OEBPS/images/{kvp.Key}", data: kvp.Value);

		if (content.OriginalPdf is { })
			WriteBinary(zip: zip, path: "OEBPS/original.pdf", data: content.OriginalPdf);
	}
	...
	{
		StringBuilder body = new(content.BodyHtml.Length + 512);
		body.AppendLine($"<h1>{escapedTitle}</h1>");

		if (includeMetaHeader)
			AppendMetaHeader(body: body, content: content, authorString: authorString);
	...
	{
		body.AppendLine(value: """<div class="meta">""");

		if (content.Authors.Count > 0)
			body.AppendLine($"""  <div class="authors">{authorString}</div>""");

		if (!IsNullOrWhiteSpace(value: content.Journal))
		{
			var dateSuffix = IsNullOrWhiteSpace(value: content.PublicationDate)
				? ""
				: $" ({WebUtility.HtmlEncode(value: content.PublicationDate)})";
			body.AppendLine(
				$"""  <div class="journal">{WebUtility.HtmlEncode(value: content.Journal)}{dateSuffix}</div>"""
			);
		}

		if (!IsNullOrWhiteSpace(value: content.Doi))
		{
			body.AppendLine(
				$"""  <div class="doi">DOI: {WebUtility.HtmlEncode(value: content.Doi)}</div>"""
			);
		}

		body.AppendLine(value: "</div>");

		if (!IsNullOrWhiteSpace(value: content.AbstractText))
		{
			body.AppendLine(
				$"""<div class="abstract"><strong>Abstract:</strong> {WebUtility.HtmlEncode(value: content.AbstractText)}</div>"""
			);
		}
	}
	...
	)
	{
		...
		var sourceTag =
			content.SourceUrl is null || IsNullOrWhiteSpace(value: content.SourceUrl.AbsoluteUri)
				? ""
				: $"<dc:source>{WebUtility.HtmlEncode(value: content.SourceUrl.AbsoluteUri)}</dc:source>";
		var publisherTag = IsNullOrWhiteSpace(value: content.Journal)
			? ""
			: $"<dc:publisher>{WebUtility.HtmlEncode(value: content.Journal)}</dc:publisher>";
		var dateTag = IsNullOrWhiteSpace(value: content.PublicationDate)
			? ""
	...
	{
		var ext = Path.GetExtension(path: filename).ToLowerInvariant();
		return ext switch
		{
	...
	{
		using Stream stream = zip.CreateEntry(entryName: path, compressionLevel: level).Open();
		using StreamWriter writer = new(stream: stream, encoding: Utf8NoBom);
