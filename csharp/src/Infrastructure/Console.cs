...
public static class Console
{
	...
	public static string SourceBadge(string source) =>
		source.Equals(value: "Discogs") ? "[yellow]Discogs[/]" : "[cyan]MusicBrainz[/]";
	...
	public static string ProgressBar(int completed, int total, string? eta = null)
	{
		...
		var bar = new string(c: '█', count: filled) + new string(c: '░', 20 - filled);
	...
	{
		var percent = total > 0 ? (double)completed / total * 100 : 0;
		var color = ProgressColor(percent: percent);
		var bar = WideProgressBar(percent: percent);
	...
	{
		var sig = IsNullOrEmpty(value: shortFlag)
			? $"    --{longFlag}"
			: $"  -{shortFlag}, --{longFlag}";
		...
		var desc = Markup.Escape(text: description);
		...
		var rawSigLen = MarkupTagPattern.Replace(input: sig, replacement: "").Length;
		var padding = Math.Max(val1: 2, 44 - rawSigLen);
	...
	private static void Write(LogLevel level, string color, string message, params object?[] args)
	{
		...
		var formatted = args.Length > 0 ? Format(message: message, args: args) : message;
		var timestamp = DateTime.Now.ToString(format: "HH:mm:ss");
		var levelCode = LevelCodes[key: level];
		AnsiConsole.MarkupLine(
