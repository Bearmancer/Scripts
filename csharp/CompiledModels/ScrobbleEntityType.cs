...
namespace MyCompiledModels
{
	...
	public static partial class ScrobbleEntityType
	{
		...
		)
		{
			...
			var id = runtimeEntityType.AddProperty(
				"Id",
				typeof(long),
				propertyInfo: typeof(Scrobble).GetProperty(
					"Id",
					BindingFlags.Public | BindingFlags.Instance | BindingFlags.DeclaredOnly
				),
				fieldInfo: typeof(Scrobble).GetField(
					"<Id>k__BackingField",
			...
			var platform = runtimeEntityType.AddProperty(
				...
				fieldInfo: typeof(Scrobble).GetField(
					"<Platform>k__BackingField",
					BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly
				)
			);
			platform.SetComparer(
			...
			var index = runtimeEntityType.AddIndex(new[] { platform });
			index.AddAnnotation("Relational:Name", "idx_scrobbles_platform");

			var index0 = runtimeEntityType.AddIndex(new[] { scrobbledAt });
			index0.AddAnnotation("Relational:Name", "idx_scrobbles_scrobbled_at");

			var index1 = runtimeEntityType.AddIndex(new[] { trackId });

			var index2 = runtimeEntityType.AddIndex(new[] { trackId, scrobbledAt }, unique: true);
		...
		)
		{
			var runtimeForeignKey = declaringEntityType.AddForeignKey(
				new[] { declaringEntityType.FindProperty("TrackId") },
			...
			var track = declaringEntityType.AddNavigation(
				"Track",
				runtimeForeignKey,
				onDependent: true,
				typeof(Track),
			...
			var scrobbles = principalEntityType.AddNavigation(
				"Scrobbles",
				runtimeForeignKey,
				onDependent: false,
