...
namespace MyCompiledModels
{
	...
	public static partial class AlbumEntityType
	{
		...
		)
		{
			...
			var releaseDate = runtimeEntityType.AddProperty(
				...
				fieldInfo: typeof(Album).GetField(
					"<ReleaseDate>k__BackingField",
					BindingFlags.NonPublic | BindingFlags.Instance | BindingFlags.DeclaredOnly
				),
				nullable: true
			);
			releaseDate.AddAnnotation(
				"Npgsql:ValueGenerationStrategy",
				NpgsqlValueGenerationStrategy.None
			);
			...
			var index = runtimeEntityType.AddIndex(new[] { artistId });

			var index0 = runtimeEntityType.AddIndex(new[] { releaseDate });
		...
		)
		{
			...
			var albums = principalEntityType.AddNavigation(
				"Albums",
				runtimeForeignKey,
				onDependent: false,
